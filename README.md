...
> **Примечание**: При первом запуске может потребоваться некоторое время для загрузки модели llama3.2 (около 4 ГБ)
...
2. Запустите скрипт для автоматического запуска:
   ```bash
   # macOS / Linux
